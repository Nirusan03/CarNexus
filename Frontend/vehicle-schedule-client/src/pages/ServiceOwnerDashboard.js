...
const ServiceOwnerDashboard = () => {
  ...
  const statusOptions = ["Pending", "In Progress", "Ready for Pickup", "Completed"];
  ...
  const fetchBookings = async () => {
    try {
      ...
      const response = await axios.get(`http://127.0.0.1:5000/booking/owner/${ownerEmail}`);
  ...
  const fetchOwnerInfo = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:5000/service/service-owners", {
        headers: { Authorization: `Bearer ${token}` },
      });
      const matchedOwner = response.data.find(owner => owner.email === userEmail);
  ...
  const updateStatus = async (bookingId) => {
    try {
      ...
      if (updatedBooking) {
        ...
        const body = `Dear Customer,\n\nYour service booking for "${updatedBooking.service_type}" has been updated.\n\nStatus: ${status}\nPickup Time: ${updatedBooking.pickup_time}\nDrop-off Time: ${updatedBooking.dropoff_time}\n\nIf you have any questions, feel free to reply to this message.\n\nBest regards,\n${ownerInfo?.owner_name || "Your Service Provider"}\nEmail: ${fromEmail}`;

        const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=${updatedBooking.customer_email}&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    ...
    <div className="account-container">
      ...
      {ownerInfo && (
        <div className="owner-info-card">
          ...
          <p><strong>Service Name:</strong> {ownerInfo.business_name}</p>
          <p><strong>Location:</strong> {ownerInfo.location}</p>
          <p><strong>Contact:</strong> {ownerInfo.contact}</p>
          <p><strong>Rating:</strong> {ownerInfo.rating}</p>
        </div>
      )}
        ...
        <div className="booking-list">
          {bookings.map((booking) => (
            <div key={booking._id} className="booking-card">
              ...
              <p><strong>Drop-off:</strong> {booking.dropoff_time}</p>
              <p><strong>Status:</strong> {booking.status}</p>
              <p><strong>From:</strong> {ownerInfo?.email}</p>
              ...
              <div className="report-section">
                ...
                {!booking.service_report && (
                  <>
                    ...
                    <textarea
                      ...
                      onChange={(e) => handleReportChange(booking._id, "issues_found", e.target.value)}
                    />
                    <textarea
                      placeholder="Work Done"
                      onChange={(e) => handleReportChange(booking._id, "work_done", e.target.value)}
                    />
                    <input
                      type="number"
                      placeholder="Total Cost"
                      onChange={(e) => handleReportChange(booking._id, "total_cost", e.target.value)}
                    />
                  </>
                )}
                ...
                <div className="action-buttons">
                  {!booking.service_report && (
                    <button className="submit-btn" onClick={() => submitReport(booking._id)}>
                      Submit Report
                    </button>
                  )}
                  <button className="status-btn" onClick={() => updateStatus(booking._id)}>
                    Update Status
                  </button>
                </div>
              </div>
              ...
              {booking.service_report && (
                <div className="report-summary">
                  ...
                  <p><strong>Issues:</strong> {booking.service_report.issues_found}</p>
                  <p><strong>Work:</strong> {booking.service_report.work_done}</p>
                  <p><strong>Cost:</strong> Rs.{booking.service_report.total_cost}</p>
                </div>
              )}
            </div>
          ))}
